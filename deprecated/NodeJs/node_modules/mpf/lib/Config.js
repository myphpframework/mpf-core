...
if (!process.env.MPF_ENV) {
  console.error('#################################################################');
  console.error('# The enviroment variable "MPF_ENV" must be set according       #');
...
function get (filename) {
  if (configFiles.hasOwnProperty(filename)) {
    return configFiles[filename].getEnv(process.env.MPF_ENV);
  }

  try {
    // TODO: must check for /etc/mpf/ .. use phproxy to get the env list of configs?
...
ConfigFile.prototype.getEnv = function (env) {
  if (!this.sections.hasOwnProperty(env)) {
